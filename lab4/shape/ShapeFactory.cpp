...
std::vector<std::string> GetArgsFromString(std::string argsString)
{
...
double MakeDouble(const std::string& numberString)
{
	try
	{
...
CPoint MakePoint(const std::string& x, const std::string& y)
{
	try
	{
		return {
...
uint32_t MakeUintFromHex(const std::string& hex)
{
	std::stringstream ss;
	ss << std::hex << hex;
	uint32_t result;
...
std::unique_ptr<CLineSegment> CreateLine(const std::vector<std::string>& args)
{
	if (args.size() < 6)
	{
...
std::unique_ptr<CRectangle> CreateRectangle(const std::vector<std::string>& args)
{
	if (args.size() < 7)
	{
...
std::unique_ptr<CTriangle> CreateTriangle(const std::vector<std::string>& args)
{
	if (args.size() < 9)
	{
...
std::unique_ptr<CCircle> CreateCircle(const std::vector<std::string>& args)
{
	if (args.size() < 6)
	{
